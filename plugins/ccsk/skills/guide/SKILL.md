---
name: guide
description: Explain how to use ccsk in Claude Code — the Build Cadence, the entry-point commands, when to use which, with worked end-to-end examples. Use when a user asks "how do I use ccsk", "what does /ccsk:execute do", "where do I start", or wants a walkthrough or use cases.
...
## What ccsk is
A **pure-markdown** Claude Code kit that runs all work on one rhythm — the **Build Cadence** — backed by a self-learning memory loop under `.ccsk/`. No hooks, no scripts; per-agent model/effort tiering. Behavior lives in skills, agents, and always-on rules. Sessions **compound**: what you learn is written back and rehydrated next time.
...
- **Forge** — implement against an agreed design, one slice at a time. (`/ccsk:execute`)
- **Prove** — focused tests; show it side-effect free. (inside `/ccsk:execute`)
- **Sign-off** — separate-reviewer verdict + evidence + memory write-back. (inside `/ccsk:execute`)
...
| Plan exists, or scope is clear and bounded | **Forge** | `/ccsk:execute` |
| Quick, obvious bugfix or scoped edit | **Forge** | `/ccsk:execute` |
...
**Golden path:** `/ccsk:rehydrate` → `/ccsk:plan` → `/ccsk:execute`. For a clear small change, skip planning and go straight to `/ccsk:execute` (it rehydrates for you).
...
- **Rehydrate before building.** `/ccsk:execute` reads memory and reconciles the ledger against real code first.
...
## Top anti-patterns
- Calling `/ccsk:execute` cold without rehydrating, then trusting stale checkboxes.
